...
import { getSemverRange } from "../../util/get-semver-range.js";
import { mergeVisitorsInPlace } from "../../util/merge-visitors-in-place.js";
import rawFeatures from "./es-syntax.json" with { type: "json" };

/** @type {{ rules: Record<string, import('../rule-module.js').RuleModule> }} */
const esxPlugin = /** @type {any} */ (esx)

/** @type {Record<string, ESSyntax>} */
const features = /** @type {Record<string, ESSyntax>} */ (rawFeatures)

const esRules = esxPlugin.rules;
/** @type {Set<string>} */
 ...
 * @returns {import('eslint').Rule.RuleListener} The defined visitor.
 */
function defineVisitor(context, options) {
    return ruleMap
        ...
        .map(rule => {
            const esRule = /** @type {import('../rule-module.js').RuleModule} */ (
                esRules[rule.ruleId]
            )
