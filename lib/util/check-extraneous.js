 ...
 */
function getPnpmWorkspacePatterns(dir) {
    const filePath = path.join(dir, "pnpm-workspace.yaml")
    let workspace

    try {
        workspace = yaml.load(fs.readFileSync(filePath, "utf8"))
    } catch {
        return []
    }
...
function isWorkspacePackage(packageInfo, workspacePackageInfo) {
    ...
    return matchesWorkspacePattern(relativePath, [
        ...getWorkspacePatterns(workspacePackageInfo.workspaces),
        ...getPnpmWorkspacePatterns(
            path.dirname(workspacePackageInfo.filePath)
        ),
    ])
}
